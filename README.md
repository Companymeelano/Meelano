## نسخهٔ ۱۲٫۰ — هستهٔ واقعی Xray

هستهٔ `libv2ray` (AndroidLibXrayLite v26.8.20) به‌عنوان موتور اصلی اتصال اضافه
شد و موتور Kotlin دست‌نویس به‌عنوان پشتیبان باقی ماند.

این همان چیزی است که **Reality** را ممکن می‌کند — ۲۰۶ نود از ۵۸۱ نود VLESS در
فید اصلی — به‌علاوهٔ `xhttp`، ترنسپورت‌های مبتنی بر QUIC و رمزهای SS2022.

### چرا Reality بدون هسته ممکن نبود
Reality «TLS با گواهی قرضی» نیست: کلاینت باید تبادل کلید X25519 با کلید عمومی
سرور انجام دهد و برچسب احراز هویت را داخل ClientHello جاسازی کند. سرور آن برچسب
را بازرسی می‌کند و در نبودش اتصال را بی‌صدا به سایت واقعی هدایت می‌کند — پس پورت
باز و TLS موفق است ولی هیچ ترافیکی عبور نمی‌کند.

### جزئیات مهندسی
- AAR (۵۹ مگابایت) و داده‌های geo (۲۷ مگابایت) در مخزن commit نمی‌شوند؛ CI
  آن‌ها را با نسخهٔ ثابت دانلود می‌کند.
- دسترسی به هسته از راه reflection است تا پروژه بدون AAR هم کامپایل شود؛
  `BuildConfig.HAS_XRAY` موتور را در زمان اجرا انتخاب می‌کند.
- APK بر اساس ABI تفکیک شد — نسخهٔ arm64 حدود یک‌چهارم نسخهٔ universal است.
- نودهای Reality در مرحلهٔ اعتبارسنجی حذف نمی‌شوند: پروب Kotlin نمی‌تواند
  آن‌ها را تأیید کند، پس تأییدشان تضمینی منفی کاذب بود.

## نسخهٔ ۹٫۰ — شبکه و آزمایش سرعت

### لایهٔ HTTP
`callTimeout` (سقف کل درخواست، که connect/read به‌تنهایی پوشش نمی‌دهند)،
HTTP/2، استخر اتصالات گرم، gzip صریح، و کش دیسک ۸ مگابایتی که `ETag` و
`Last-Modified` را رعایت می‌کند — فهرست بدون تغییر یک پاسخ ۳۰۴ هزینه دارد.

تلاش مجدد با backoff نمایی، ولی **فقط برای خطاهای واقعاً گذرا** (۴۲۹، ۵xx،
خطای شبکه). آینهٔ مسدود یا ناموجود (۴۰۳/۴۰۴) با تکرار بهتر نمی‌شود، و پس از
آخرین آینه هم صبر نمی‌کنیم.

### آزمایش سرعت واقعی
`SpeedTester` یک فایل را **از داخل تونل همان سرور** دانلود می‌کند و بایت‌های
واقعاً رسیده را گزارش می‌دهد. تأخیر و پهنای باند رابطهٔ سستی دارند — نودی با
پینگ ۴۰ میلی‌ثانیه می‌تواند بسیار کند باشد — پس هر عددی که از پینگ استنتاج شود
حدس است که به‌جای واقعیت ارائه شده. مقیاس عقربه لگاریتمی است چون سرعت‌ها چند
مرتبهٔ بزرگی را می‌پوشانند.
